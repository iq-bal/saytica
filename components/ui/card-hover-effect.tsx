      ...
      {items.map((item, idx) => (
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 h-full w-full bg-muted/50 dark:bg-slate-800/[0.8] block rounded-3xl"
...
export const Card = ({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl h-full w-full p-4 overflow-hidden bg-card border border-border/50 group-hover:border-border relative z-20",
        className
      )}
    >
