...
const jobs: Job[] = [
  ...
  },
  {
    ...
    description: "Help us ship polished product experiences on a project basis.",
    category: "Development",
    link: "/careers/freelance",
  },
  {
    id: "customer-success-freelance",
      ...
      <section className="max-w-5xl mx-auto px-4 md:px-6 pt-10 md:pt-14">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-2">
          {categories.map((cat) => {
            ...
            return (
              <button key={cat} onClick={() => setActiveCategory(cat)} className="outline-none" aria-pressed={isActive}>
                <Badge variant={isActive ? "secondary" : "outline"} className={isActive ? "rounded-full bg-neutral-900 text-white px-3 py-1" : "rounded-full px-3 py-1"}>
                  {cat}
                </Badge>
              </button>
            )
            ...
            <motion.div key={job.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: idx * 0.06 }} className="py-6 md:py-7">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-gray-600 text-sm md:text-base">{job.description}</p>
                  <div className="mt-3 flex items-center gap-2">
                    <Badge variant="outline" className="rounded-full px-3 py-1 text-xs inline-flex items-center gap-1"><MapPin className="size-3" /> 100% remote</Badge>
                    <Badge variant="outline" className="rounded-full px-3 py-1 text-xs inline-flex items-center gap-1"><Clock className="size-3" /> Full-time</Badge>
                  </div>
                </div>
                <Link href={job.link} className="text-base md:text-lg text-foreground inline-flex items-center gap-1 hover:underline">
