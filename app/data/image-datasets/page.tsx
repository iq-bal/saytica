...
import { ArrowRight, Database, Globe, Search, Filter, Image as ImageIcon } from "lucide-react"
...
const makeDatasets = (count = 72): Dataset[] => {
  ...
  const topics = ["Objects", "Scenes", "Medical", "Retail", "Traffic", "Satellite"]
  ...
  return Array.from({ length: count }).map((_, i) => {
    ...
    return {
      id: `mdt-img-${i + 1}`,
      title: `MDT-IM-${String(i + 1).padStart(3, "0")} ${lang} Image Corpus`,
      openSource: isOpen,
      samples: 2000 + i * 50,
      language: lang,
      industry: ind,
...
const TOPICS = ["Objects", "Scenes", "Medical", "Retail", "Traffic", "Satellite"] as const

export default function ImageDatasetsPage() {
  const [allDatasets] = useState<Dataset[]>(() => makeDatasets(72))
  const [query, setQuery] = useState("")
  const [activeIndustry, setActiveIndustry] = useState<string | null>(null)
  const [activeTypeToggle, setActiveTypeToggle] = useState<(typeof DATA_TYPES)[number]>("Image Datasets")
    ...
    <div className="relative">
      <section className="pt-10 md:pt-14">
        <Badge variant="outline" className="rounded-full px-3 py-1 text-xs md:text-sm inline-flex items-center gap-2">
          <ImageIcon className="h-3 w-3" /> Image Datasets
        </Badge>
        <h1 className="mt-4 text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight">Explore Image Corpora</h1>
        <p className="mt-3 text-muted-foreground leading-relaxed text-sm md:text-base lg:text-lg max-w-2xl">
          Browse multilingual image datasets across industries, applications, and regions.
        </p>
      ...
      <section className="mt-14 md:mt-18">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="rounded-xl border">
            ...
            <CardContent className="text-sm text-muted-foreground">Wide-ranging image datasets across domains and use-cases.</CardContent>
          </Card>
          <Card className="rounded-xl border">
            <CardHeader>
      ...
      <section className="m-12 md:mt-16">
        <div className="rounded-xl border p-6 md:p-8 bg-muted/40">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              <h2 className="text-xl md:text-2xl font-semibold">Need specific image datasets?</h2>
