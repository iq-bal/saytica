...
import { ArrowRight, Database, Globe, Search, Filter, Layers } from "lucide-react"
...
const makeDatasets = (count = 72): Dataset[] => {
  ...
  const topics = ["Work", "Daily Life", "Science & Technology", "Smart Devices", "Retail", "Healthcare"]
  ...
  return Array.from({ length: count }).map((_, i) => {
    ...
    return {
      id: `mdt-lex-${i + 1}`,
      title: `MDT-LX-${String(i + 1).padStart(3, "0")} ${lang} Lex Resource`,
      openSource: isOpen,
      samples: 500 + i * 20,
      language: lang,
      industry: ind,
...
export default function LexPage() {
  ...
  const [activeTypeToggle, setActiveTypeToggle] = useState<(typeof DATA_TYPES)[number]>("Lex")
    ...
    <div className="relative">
      <section className="pt-10 md:pt-14">
        <Badge variant="outline" className="rounded-full px-3 py-1 text-xs md:text-sm inline-flex items-center gap-2">
          <Layers className="h-3 w-3" /> Lex
        </Badge>
        <h1 className="mt-4 text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight">Explore Lex Resources</h1>
        <p className="mt-3 text-muted-foreground leading-relaxed text-sm md:text-base lg:text-lg max-w-2xl">
          Browse multilingual lexicons, dictionaries, taxonomies, and terminology datasets.
        </p>
            ...
            <Card key={d.id} className="rounded-xl border border-border h-full flex flex-col">
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Link href={`/datasets/lex/${d.id}`} className="text-sm inline-flex items-center gap-1 hover:underline">
      ...
      <section className="mt-14 md:mt-18">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="rounded-xl border">
            ...
            <CardContent className="text-sm text-muted-foreground">Extensive lex resources spanning languages and domains.</CardContent>
          </Card>
          <Card className="rounded-xl border">
            <CardHeader>
              <CardTitle className="text-lg">Quality</CardTitle>
            </CardHeader>
            <CardContent className="text-sm text-muted-foreground">Curated dictionaries and taxonomies with review flows.</CardContent>
          </Card>
          <Card className="rounded-xl border">
            <CardHeader>
              <CardTitle className="text-lg">Scale</CardTitle>
            </CardHeader>
            <CardContent className="text-sm text-muted-foreground">Proven pipelines for enterprise terminology resources.</CardContent>
          </Card>
        </div>
      </section>
      ...
      <section className="m-12 md:mt-16">
        <div className="rounded-xl border p-6 md:p-8 bg-muted/40">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              <h2 className="text-xl md:text-2xl font-semibold">Need specific lex resources?</h2>
