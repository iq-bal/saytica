...
import { Layers, Globe, Download, Database, ArrowRight } from "lucide-react";
...
type DatasetDetails = {
  ...
  openSourceEntries?: string;
  related?: Array<{ title: string; href: string }>;
};

const MDT_LX_SAMPLE: DatasetDetails = {
  id: "mdt-lex-1",
  title: "MDT-LX-001 Multilingual Lex Resource",
  sourceUrl: "#",
  licenseNote: "Open Source Training Datasets Terms of Use apply to this dataset.",
  downloadUrl: "#",
  language: "English",
  resourceType: "Lexicon / Ontology",
  entries: "5M entries",
  contentDescription: "Entity dictionaries, terminology lists, and taxonomies.",
  fileFormat: "CSV, JSON",
  applicationField: "NLP / Search",
  sensitiveItems: "nil",
  copyrightOwner: "Magic Data",
  openSourceEntries: "100k entries",
  related: [
    { title: "MDT-LX-002 Mandarin Chinese Terminology", href: "/data/lex/mdt-lex-2" },
    { title: "MDT-LX-003 German Taxonomy", href: "/data/lex/mdt-lex-3" },
  ],
};

function getDatasetDetails(slug: string): DatasetDetails {
  if (["mdt-lex-1", "mdt-lx-001", "mdt-lx-sample"].includes(slug.toLowerCase())) {
    return MDT_LX_SAMPLE;
  }
  return {
    ...MDT_LX_SAMPLE,
    id: slug.toLowerCase(),
    title: `${slug.toUpperCase()} Lex Resource`,
  };
}
...
function makeDummyListing(title: string, language = "English"): ListingDataset {
  return {
    ...
    language,
    industry: "Smart Home",
    application: "Keyword Lists",
    type: "Training Set",
    region: "USA",
  };
}

export default async function LexDatasetDetailPage({
  params,
}: {
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/30 dark:from-background dark:to-muted/20">
      ...
      <div className="sticky top-0 z-10 backdrop-blur-md border-b border-border/50 px-4 py-3 flex items-center justify-between">
        <nav className="text-sm text-muted-foreground flex items-center space-x-2">
          <Link href="/data/lex" className="hover:text-foreground">
            Lex
          </Link>
          <span>/</span>
      ...
      <section className="max-w-6xl mx-auto px-6 pt-10 pb-6 text-center">
        <div className="inline-flex items-center gap-2 rounded-full border border-border px-4 py-1 text-sm">
          <Layers className="w-4 h-4" /> Lex Resource Detail
        </div>
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-2">
          ...
          <Badge variant="outline">Entries: {d.entries}</Badge>
          {d.openSourceEntries && (
            <Badge variant="outline">Open Source: {d.openSourceEntries}</Badge>
          )}
        </div>
      </section>
        ...
        <div className="overflow-x-auto rounded-xl border border-border bg-card/60 backdrop-blur">
          <table className="w-full text-sm">
            <tbody>
              ...
              <tr className="border-b border-border">
                <th className="w-1/3 text-left p-3 font-medium">Resource Type</th>
                <td className="p-3 text-muted-foreground">{d.resourceType}</td>
              </tr>
              <tr className="border-b border-border">
                <th className="w-1/3 text-left p-3 font-medium">Entries</th>
                <td className="p-3 text-muted-foreground">{d.entries}</td>
              </tr>
              <tr className="border-b border-border">
      ...
      <section className="max-w-6xl mx-auto px-6 mb-12">
        ...
        {(() => {
          const sample = makeDummyListing("MDT-SAMPLE Dummy Lex Resource", d.language);
          return (
